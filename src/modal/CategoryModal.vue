    ...
    <div ref="modalRef">
      ...
      >
        <div className="flex flex-col justify-between h-[13.875rem] w-[12.625rem]">
          <button
            class="flex items-center w-full h-10 gap-2 pl-9 hover:bg-background-200"
            @click="routeTo('recommend')"
          >
            <icon name="recommend" />
            <span class="text-xs font-bold">금융 상품 추천받기</span>
          </button>
          <button class="flex items-center w-full h-10 gap-2 pl-9 hover:bg-background-200" @click="routeTo('compare')">
            <icon name="compare" />
            <span class="text-xs font-bold">예적금 금리 비교하기</span>
          </button>
          <button class="flex items-center w-full h-10 gap-2 pl-9 hover:bg-background-200" @click="routeTo('exchange')">
            <icon name="exchange" />
            <span class="text-xs font-bold">환율 계산하기</span></button
          ><button class="flex items-center w-full h-10 gap-2 pl-9 hover:bg-background-200" @click="routeTo('search')">
            <icon name="search" />
            <span class="text-xs font-bold">주변 은행 찾아보기 </span>
          </button>
        </div>
      </div>
    </div>
